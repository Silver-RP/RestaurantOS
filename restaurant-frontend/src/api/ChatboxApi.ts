import { ChatMessage, ChatSessionResponse, GetMessagesResponse, SendMessageAPIRequest } from '@/types/Chatbox.type';
...
export const getMessages = async (chatId: string, before?: string): Promise<ChatMessage[]> => {
  const params = before ? { before } : {};
  const { data } = await axiosInstance.get<GetMessagesResponse>(`/chat/${chatId}/messages`, { params });
  return data.messages;
};

// 3. Gửi tin nhắn mới
export const sendMessage = async ({ chatId, content, replyTo }: SendMessageAPIRequest): Promise<ChatMessage> => {
  const { data } = await axiosInstance.post(`/chat/${chatId}/message`, { content, replyTo });
  console.log('Sent message:', data.message);
  return data.message;
};

// Cashier lấy hoặc tạo phiên chat với user cụ thể
export const getUserChatSession = async (userId: string): Promise<ChatSessionResponse> => {
