...
const MyReservationsPage: React.FC = () => {
  ...
  return (
    <>
      <BreadcrumbComponent />
      <Container>
        <div className="w-full mx-auto min-h-[calc(100vh-568px)] py-10 text-white space-y-6">
          <NavigationReservation activeTab={activeTab} onTabChange={(tab) => {
            setActiveTab(tab);
            setPage(1);
          }} />
          ...
          {loading ? (
            ...
            <div className="flex flex-col justify-center items-center h-full py-24">
              ...
              <p className="text-gray-400 text-lg">Bạn chưa có đơn đặt bàn nào.</p>
            </div>
          ) : (
            <div className="space-y-8 w-full mx-auto">
              {reservations.map((res) => (
                <ReservationCard key={res._id} reservation={res} onView={() => setSelectedId(res._id!)} />
              ))}
            </div>
          )}
        </div>
      ...
      {selectedId && (
        <ReservationDetailModal reservationId={selectedId} onClose={() => setSelectedId(null)} />
      )}
    </>
  );
};
