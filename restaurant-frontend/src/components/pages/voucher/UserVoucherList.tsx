      ...
      <Container className="flex gap-6 sm:py-10">
        ...
        <div className="flex-1 w-2/3 bg-bodyBackground p-10 border border-[#FFE0A0]">
          ...
          {isLoading ? (
            ...
            <div className="text-center text-lg text-secondaryColor">
              ...
              {vouchers.map((voucher) => {
                ...
                return (
                  <div
                    ...
                    className={`relative rounded-xl bg-[#0A1F2C] text-white p-7 shadow-lg flex flex-col gap-3 h-[190px] w-full min-w-[260px] max-w-[340px] justify-between items-center mx-auto ${isOutOfStockButValid || isDeleted ? 'opacity-50' : ''}`}
                  >
                    <div>
                      <div className="flex justify-between items-center mb-2">
                        <span className="text-base font-semibold tracking-wide text-[#FFDA95]">
                          {voucher.code}
                        </span>
                        <span
                          className={`text-xs px-2 py-1 rounded ${isDeleted ? 'bg-red-700' : 'bg-gray-700'}`}
                        >
                          {isDeleted
                            ? 'Đã bị thu hồi'
                            : statusMap[voucher.user_voucher_status] ||
                              voucher.user_voucher_status}
                        </span>
                      </div>
                        ...
                        {voucher.max_discount_value &&
                          voucher.discount_type === 'percent' && (
                            <span>
                              {' '}
                              (Tối đa{' '}
                              {voucher.max_discount_value.toLocaleString()} VNĐ)
                            </span>
                          )}
                      </div>
                      {Number(voucher.min_order_value) > 0 && (
                        <div className="text-xs mb-1">
                          ...
                          {Number(voucher.min_order_value).toLocaleString()} VNĐ
                        </div>
                      )}
                    </div>
                    ...
                    {(isOutOfStockButValid || isDeleted) && (
                      <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                        {isDeleted ? (
                          <span className="bg-black bg-opacity-70 text-white text-xs px-3 py-1 rounded">
                            Voucher này đã bị admin thu hồi và không còn hiệu
                            lực
                          </span>
                        ) : (
                          <span className="bg-black bg-opacity-70 text-white text-xs px-3 py-1 rounded">
