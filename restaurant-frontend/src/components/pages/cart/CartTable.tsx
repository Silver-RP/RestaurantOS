...
const CartTable: React.FC<CartTableProps> = ({
  ...
  const toggleSelectAll = () => {
    const selectableItems = items.filter(
      (item) => !item.stockStatus.isDisabled,
    );
    ...
    <div className="space-y-4 md:space-y-0">
        ...
        <table className="w-full min-w-[600px] text-xs sm:text-sm border-separate border-spacing-y-1">
          <thead className="sticky top-0 z-20 bg-bodyBackground">
            <tr className="text-white">
              <th className="px-2 sm:px-4 text-center w-[40px]">
                <input
                  ...
                  checked={
                    ...
                    selectedIds.length ===
                      items.filter((i) => !i.stockStatus.isDisabled).length
                  }
                />
              </th>
              <th className="py-2 sm:py-3 text-left text-base sm:text-lg font-light">
                Sản phẩm
              </th>
              <th className="py-2 sm:py-3 text-left text-base sm:text-lg font-light">
                Giá
              ...
              <th className="py-2 sm:py-3 text-center text-base sm:text-lg font-light">
                Xoá
              </th>
            </tr>
          </thead>
      ...
      <div className="block md:hidden space-y-4">
        {items.map((item) => (
          <div
            key={item.id}
            className="bg-[#0D3343]/50 border border-[#26455E] rounded shadow-sm p-4 flex gap-4"
          >
            <input
              type="checkbox"
            ...
            <div className="flex-1 flex flex-col justify-between text-white">
              ...
              <div className="mt-2 flex items-center justify-between">
                <div className="flex items-center border border-gray-600 rounded">
                  <button className="px-2 py-1 hover:bg-secondaryColor hover:text-black">
                    −
                  </button>
                ...
                <button className="text-red-400 text-lg hover:text-red-600">
                  ×
                </button>
              </div>
            </div>
