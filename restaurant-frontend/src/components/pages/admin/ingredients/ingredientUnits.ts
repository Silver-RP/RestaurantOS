...
export function getIngredientUnitLabel(value: string): string {
  return (
    ingredientUnits.find((unit) => unit.value === value)?.label || value
  );
}
...
const getStockStatus = (
  ...
  if (count <= 0) return { label: '❌ Hết hàng', color: 'red' };

  const threshold = customThreshold ?? unitThresholds[unit] ?? 10;

  if (count <= threshold) return { label: '⚠ Sắp hết', color: 'orange' };
