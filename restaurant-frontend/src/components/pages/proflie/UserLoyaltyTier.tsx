...
const UserLoyaltyTier: React.FC = () => {
  ...
  const { data: userVouchers = [], refetch: refetchUserVouchers } =
    useUserVouchers();
  const [milestoneLoading, setMilestoneLoading] = useState(false);
  const [milestoneError, setMilestoneError] = useState('');
  const [savingVoucherId, setSavingVoucherId] = useState<string | null>(null);

  // Detect mobile screen to show compact labels under the progress bar
  const [isMobile, setIsMobile] = useState(false);
  useEffect(() => {
    if (typeof window === 'undefined') return;
    const mql = window.matchMedia('(max-width: 640px)');
    const handler = (e: MediaQueryListEvent | MediaQueryList) => {
      // both event and initial mql have .matches
  ...
  const formatAmountCompact = (value: number) => {
    if (!isMobile) return `${value.toLocaleString()}đ`;
    if (value === 0) return '0';
  ...
  useEffect(() => {
    Promise.all([
      ...
      .then(([info, tiers, milestones]) => {
        ...
        setTiers(
          tiers.sort(
            (a: LoyaltyTier, b: LoyaltyTier) => a.min_spent - b.min_spent,
          ),
        );
  ...
  const currentTierIdx =
    info.current_tier && tiers.length > 0
  ...
  const progressPercent =
    ((currentTierIdx + percentToNext / 100) / (tiers.length - 1)) * 100;
  ...
  return (
    <div className="mb-8">
      <h3 className="text-xl font-semibold text-[#FFDA95] mb-4">
        ...
        {tiers.map((tier, idx) => {
          ...
          return (
            ...
            >
              {/* Tên hạng */}
              <div
                className={`text-sm font-bold mb-1 text-center ${isCurrent || isDefaultNew ? 'text-yellow-400' : 'text-gray-300'}`}
                style={{ minHeight: 20 }}
              >
              ...
              />
              {/* Số tiền (compact on mobile) */}
              <div className="text-xs text-gray-400 text-center mt-1">
                {formatAmountCompact(tier.min_spent)}
              </div>
            </div>
          );
        })}
      ...
      <div className="flex flex-col lg:flex-row gap-6 mt-4">
        <div>
          ...
          <span className="lg:text-lg text-sm font-bold text-white">
            {info.total_points ?? 0}
          </span>
        </div>
        <div>
          ...
          <span className="lg:text-lg text-sm font-bold text-white">
            {currentSpent.toLocaleString()} VNĐ
          </span>
        </div>
        <div>
          <span className="block text-xs text-gray-400">
          ...
          <span className="lg:text-lg text-sm font-bold text-white">
            {info.total_spent?.toLocaleString()} VNĐ
          </span>
        </div>
        <div>
          <span className="block text-xs text-gray-400">Quyền lợi</span>
          <span className="text-sm text-white">
            {info.current_tier?.benefits ?? '---'}
          </span>
        </div>
        <div className="ml-5">
          {milestones.length > 0 && (
            <FaGift
              className="text-yellow-400 cursor-pointer text-4xl"
              title="Xem mốc quà tặng"
          ...
          <div className="flex flex-wrap gap-3">
            {Object.entries(info.yearly_spending)
              ...
              .map(([year, value]) => (
                <div
                  key={year}
                  className="bg-gray-800 rounded px-3 py-1 text-xs text-white"
                >
      ...
      {nextTier && (
        <div className="mt-2 text-xs text-yellow-300">
          Còn {(nextTier.min_spent - currentSpent).toLocaleString()}đ để lên
          hạng <b>{getTierNameVN(nextTier.tier_name)}</b>
        </div>
      )}
      {/* Modal mốc quà tặng */}
      {showMilestoneModal && (
        <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50">
          <div className="bg-bodyBackground rounded-lg shadow-lg p-6 w-full max-w-lg relative">
            ...
            <h4 className="text-lg font-bold mb-4 text-secondaryColor">
              Các mốc quà tặng
            </h4>
            {milestoneLoading ? (
              <div>Đang tải...</div>
              ...
              <div className="space-y-4 max-h-96 overflow-y-auto">
                {milestones.map((milestone) => {
                  ...
                  const userVoucherIds = userVouchers.map((v) =>
                  ...
                  return (
                    <div
                      key={milestone._id}
                      className="border border-secondaryColor rounded p-3 flex flex-col md:flex-row md:items-center justify-between gap-2"
                    >
                      <div>
                        ...
                        <div className="text-xs text-white">
                          Voucher:{' '}
                          <span className="font-mono text-secondaryColor">
                      ...
                      <div className="flex flex-col items-end gap-2 min-w-[120px]">
                        {alreadySaved ? (
                          <button
                            className="px-3 py-1 rounded bg-gray-300 text-gray-600 cursor-not-allowed"
                            disabled
                          >
                            Đã lưu
                          </button>
