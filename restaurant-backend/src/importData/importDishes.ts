...
const run = async () => {
  ...
  for (const item of data) {
    ...
    const discount_price = discountChance > 0.5 ? Math.floor(price * (1 - getRandomInt(5, 30) / 100)) : null
    ...
    if (rating_count > 0) {
      rating = Array.from({ length: rating_count }, () => getRandomInt(3, 5))
                    .reduce((a, b) => a + b, 0)
    ...
    const imagePaths = [
      ...imageExtensions.map(ext => path.join(__dirname, 'menu_images/mon_chinh', `${baseImageName}${ext}`)),
      ...imageExtensions.map(ext => path.join(__dirname, 'menu_images/mon_chinh', `${baseImageName}_1${ext}`)),
      ...imageExtensions.map(ext => path.join(__dirname, 'menu_images/mon_chinh', `${baseImageName}_2${ext}`)),
      ...imageExtensions.map(ext => path.join(__dirname, 'menu_images/mon_chinh', `${baseImageName}_3${ext}`)),
    ]
